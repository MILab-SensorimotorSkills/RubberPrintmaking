...
//         [Header("Textures")]
//         public Texture advanceTexture;  // 추가된 텍스처 변수
//         [SerializeField] private Texture thirdTexture;// 3번 키를 위한 추가 텍스처 변수
//         private Texture originalTexture;  // 원래 텍스처를 저장할 변수
//         private bool isUsingAdvanceTexture = false;  // 현재 어떤 텍스처를 사용 중인지 추적하는 변수
...
//                 ActivateAdvance();
//             }
//             else if (forceState && Input.GetKeyDown(KeyCode.Alpha3))
//             {
//                 ChangeToThirdTexture();  // 3번 키를 눌렀을 때 새로운 텍스처로 변경
//             }
//             else if (forceState && Input.GetKeyDown(KeyCode.T))
//             {
//                 // ResetToForceState();
...
//                 var mr = next.gameObject.GetComponent<MeshRenderer>();
...
//             var inv = effector.GetComponentInParent<Inverse3Controller>();
//             var cursorMr = inv != null ? inv.Cursor.GetComponent<MeshRenderer>() : null;
//             if (cursorMr != null)
//                 cursorMr.material = effector.forceEnabled ? enabledForceMaterial : disabledForceMaterial;
...
//             }
//             isUsingAdvanceTexture = false; // 상태 초기화
//         }
//     }
// }
